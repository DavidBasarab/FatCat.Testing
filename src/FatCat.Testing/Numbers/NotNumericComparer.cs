	...
	where T : INumber<T>
{
	...
	{
		if (Subject < T.Zero) { CompareException.New(because ?? $"{Subject} should not be negative"); }
	...
	{
		if (Subject > T.Zero) { CompareException.New(because ?? $"{Subject} should not be positive"); }
	...
	{
		if (Subject == T.Zero) { CompareException.New(because ?? $"{Subject} should not be zero"); }
