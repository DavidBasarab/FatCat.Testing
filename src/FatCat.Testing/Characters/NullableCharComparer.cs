...
public class NullableCharComparer(char? subject) : ComparerBase<char?, NullableCharComparer>(subject)
{
	...
	{
		if (!Subject.HasValue || !char.IsControl(Subject.Value)) { CompareException.New(because ?? $"{SubjectDisplay} should be a control character"); }
	...
	{
		if (!Subject.HasValue || !char.IsDigit(Subject.Value)) { CompareException.New(because ?? $"{SubjectDisplay} should be a digit"); }
	...
	{
		if (!Subject.HasValue || !char.IsLetter(Subject.Value)) { CompareException.New(because ?? $"{SubjectDisplay} should be a letter"); }
	...
	{
		if (!Subject.HasValue || !char.IsLetterOrDigit(Subject.Value)) { CompareException.New(because ?? $"{SubjectDisplay} should be a letter or digit"); }
	...
	{
		if (!Subject.HasValue || !char.IsLower(Subject.Value)) { CompareException.New(because ?? $"{SubjectDisplay} should be lower cased"); }
	...
	{
		if (!Subject.HasValue || !char.IsUpper(Subject.Value)) { CompareException.New(because ?? $"{SubjectDisplay} should be upper cased"); }
	...
	{
		if (!Subject.HasValue || !char.IsWhiteSpace(Subject.Value)) { CompareException.New(because ?? $"{SubjectDisplay} should be white space"); }
