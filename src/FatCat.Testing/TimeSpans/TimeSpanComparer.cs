...
public class TimeSpanComparer(TimeSpan subject) : ComparerBase<TimeSpan, TimeSpanComparer>(subject)
{
	...
	{
		if (Math.Abs((Subject - expected).Ticks) > precision.Ticks) { CompareException.New(because ?? $"{Subject} should be within {precision} of {expected}"); }
	...
	{
		if (Subject > expected) { CompareException.New(because ?? $"{Subject} should be less than or equal to {expected}"); }
	...
	{
		if (Subject >= TimeSpan.Zero) { CompareException.New(because ?? $"{Subject} should be negative"); }
	...
	{
		if (Subject <= TimeSpan.Zero) { CompareException.New(because ?? $"{Subject} should be positive"); }
	...
	{
		if (Subject.Days != expected) { CompareException.New(because ?? $"{Subject} should have days {expected}"); }
	...
	{
		if (Subject.Hours != expected) { CompareException.New(because ?? $"{Subject} should have hours {expected}"); }
	...
	{
		if (Subject.Milliseconds != expected) { CompareException.New(because ?? $"{Subject} should have milliseconds {expected}"); }
	...
	{
		if (Subject.Minutes != expected) { CompareException.New(because ?? $"{Subject} should have minutes {expected}"); }
	...
	{
		if (Subject.Seconds != expected) { CompareException.New(because ?? $"{Subject} should have seconds {expected}"); }
