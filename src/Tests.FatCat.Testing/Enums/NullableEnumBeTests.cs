...
public class NullableEnumBeTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((TestStatus?)null).Should().Be(TestStatus.Active), "null should be Active");
	}
	...
	public void GoodBe()
	{
		((TestStatus?)TestStatus.Active).Should().Be(TestStatus.Active);
	}

	[Fact]
	public void GoodNotBe()
	{
		((TestStatus?)TestStatus.Active).Should().Not.Be(TestStatus.Inactive);
	}

	[Fact]
	public void GoodNotBeWhenNull()
	{
		((TestStatus?)null).Should().Not.Be(TestStatus.Active);
	}
}
