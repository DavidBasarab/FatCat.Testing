...
public class NullableStringMatchRegexTests : BaseTest
{
	...
	[Fact]
	public void BadMatchRegexStringWhenNoMatch() { RunCompareFailTest(() => ((string?)"hello").Should().MatchRegex("\\d+"), "hello should match regex \\d+"); }
	...
	[Fact]
	public void GoodMatchRegexIgnoreCase() { ((string?)"HELLO").Should().MatchRegex(new Regex("hello", RegexOptions.IgnoreCase)); }

	[Fact]
	public void GoodMatchRegexObject() { ((string?)"hello123").Should().MatchRegex(new Regex("\\d+")); }

	[Fact]
	public void GoodMatchRegexString() { ((string?)"hello123").Should().MatchRegex("\\d+"); }

	[Fact]
	public void GoodNotMatchRegexObject() { ((string?)"hello").Should().Not.MatchRegex(new Regex("\\d+")); }

	[Fact]
	public void GoodNotMatchRegexObjectWhenNull() { ((string?)null).Should().Not.MatchRegex(new Regex("\\d+")); }

	[Fact]
	public void GoodNotMatchRegexString() { ((string?)"hello").Should().Not.MatchRegex("\\d+"); }
