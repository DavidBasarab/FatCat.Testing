...
public class NullableGuidBeTests : BaseTest
{
	...
	[Fact]
	public void BadBeWithBecause() { RunCompareFailTest(() => ((Guid?)TestGuid).Should().Be(Guid.Empty, "custom because"), "custom because"); }
	...
	[Fact]
	public void BadNotBeWithBecause() { RunCompareFailTest(() => ((Guid?)TestGuid).Should().Not.Be(TestGuid, "custom because"), "custom because"); }

	[Fact]
	public void GoodBe() { ((Guid?)TestGuid).Should().Be(TestGuid); }

	[Fact]
	public void GoodNotBe() { ((Guid?)TestGuid).Should().Not.Be(Guid.Empty); }

	[Fact]
	public void GoodNotBeWhenNull() { ((Guid?)null).Should().Not.Be(Guid.Empty); }
}