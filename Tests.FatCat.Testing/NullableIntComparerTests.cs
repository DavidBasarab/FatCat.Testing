...
public class NullableIntComparerTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((int?)4).Should().Not.Match(x => x % 2 == 0), "4 should not match the predicate");
	}

	[Fact]
	public void BadNotMatchWithBecause()
	{
		RunCompareFailTest(() => ((int?)4).Should().Not.Match(x => x % 2 == 0, "custom because"), "custom because");
	}
