...
public class NullableStringBeLowerCasedTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((string?)"HELLO").Should().BeLowerCased(), "HELLO should be lower cased");
	}
	...
	{
		RunCompareFailTest(() => ((string?)"HELLO").Should().BeLowerCased("custom because"), "custom because");
	}

	[Fact]
	public void BadNotBeLowerCased()
	{
		RunCompareFailTest(() => ((string?)"hello").Should().Not.BeLowerCased(), "hello should not be lower cased");
	}

	[Fact]
	public void BadNotBeLowerCasedWithBecause()
	{
		RunCompareFailTest(() => ((string?)"hello").Should().Not.BeLowerCased("custom because"), "custom because");
	}

	[Fact]
	public void GoodBeLowerCased()
	{
		((string?)"hello").Should().BeLowerCased();
	}
