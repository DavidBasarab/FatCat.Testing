...
public class NullableStringBeNullTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((string?)"hello").Should().BeNull(), "hello should be null");
	}

	[Fact]
	public void BadBeNullWithBecause()
	{
		RunCompareFailTest(() => ((string?)"hello").Should().BeNull("custom because"), "custom because");
	}
	...
	{
		((string?)"hello").Should().Not.BeNull();
	}
}
