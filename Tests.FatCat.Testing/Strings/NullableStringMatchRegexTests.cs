...
public class NullableStringMatchRegexTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((string?)"hello").Should().MatchRegex(new Regex("\\d+")), "hello should match regex \\d+");
	}
	...
	{
		RunCompareFailTest(() => ((string?)"hello").Should().MatchRegex("\\d+", "custom because"), "custom because");
	}
	...
	{
		RunCompareFailTest(() => ((string?)"hello123").Should().Not.MatchRegex("\\d+"), "hello123 should not match regex \\d+");
	}

	[Fact]
	public void BadNotMatchRegexWithBecause()
	{
		RunCompareFailTest(() => ((string?)"hello123").Should().Not.MatchRegex("\\d+", "custom because"), "custom because");
	}
