...
public class NullableStringBeEquivalentToTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((string?)"hello").Should().BeEquivalentTo("world"), "hello should be equivalent to world");
	}

	[Fact]
	public void BadBeEquivalentToWhenNull()
	{
		RunCompareFailTest(() => ((string?)null).Should().BeEquivalentTo("world"), "null should be equivalent to world");
	}
	...
	{
		((string?)"hello").Should().BeEquivalentTo("hello");
	}

	[Fact]
	public void GoodBeEquivalentToIgnoreCase()
	{
		((string?)"hello").Should().BeEquivalentTo("HELLO", Options.IgnoreCase);
	}

	[Fact]
	public void GoodNotBeEquivalentTo()
	{
		((string?)"hello").Should().Not.BeEquivalentTo("world");
	}
}
