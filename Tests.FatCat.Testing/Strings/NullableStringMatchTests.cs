...
public class NullableStringMatchTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((string?)"hello").Should().Match("world*", because: "custom because"), "custom because");
	}

	[Fact]
	public void BadNotMatch()
	{
		RunCompareFailTest(() => ((string?)"hello world").Should().Not.Match("hello*"), "hello world should not match hello*");
	}
