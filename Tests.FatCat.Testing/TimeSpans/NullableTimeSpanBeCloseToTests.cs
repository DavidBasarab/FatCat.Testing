...
public class NullableTimeSpanBeCloseToTests : BaseTest
{
	...
	public void BadBeCloseToWithBecause()
	{
		...
		RunCompareFailTest(
			() => ((TimeSpan?)span).Should().BeCloseTo(TimeSpan.FromHours(3), TimeSpan.FromMinutes(30), "custom because"),
			"custom because"
		);
	}
	...
	public void GoodBeCloseTo()
	{
		...
		((TimeSpan?)span).Should().BeCloseTo(TimeSpan.FromHours(1).Add(TimeSpan.FromMinutes(15)), TimeSpan.FromMinutes(30));
	}
