...
public class CustomComparerTests : BaseTest
{
	...
	{
		new FakeWebResponse { StatusCode = 200 }
			.Should()
			.BeOk();
	}
	...
	{
		RunCompareFailTest(() =>
			new FakeWebResponse { StatusCode = 500 }
				.Should()
				.BeOk()
		);
	}
		...
		RunCompareFailTest(
			() =>
				new FakeWebResponse { StatusCode = 500 }
					.Should()
					.BeOk(),
			"status code 500 should be 200 (OK)"
		);
	}
	...
	{
		new FakeWebResponse { StatusCode = 404 }
			.Should()
			.BeNotFound();
	}
	...
	[Fact]
	public void GoodCustomComparerUsesInheritedBeOfType()
	{
		new FakeWebResponse { StatusCode = 200 }
			.Should()
			.BeOfType<FakeWebResponse>();
	}
