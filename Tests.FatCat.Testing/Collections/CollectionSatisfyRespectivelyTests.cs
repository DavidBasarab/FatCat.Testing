...
public class CollectionSatisfyRespectivelyTests : BaseTest
{
	...
	{
		new List<int> { 1, 2 }.Should().SatisfyRespectively(value => value.Should().Be(1), value => value.Should().Be(2));
	}

	[Fact]
	public void BadSatisfyRespectivelyCountMismatch()
	{
		RunCompareFailTest(() => new List<int> { 1, 2 }.Should().SatisfyRespectively(value => value.Should().Be(1)));
	}
	...
	{
		RunCompareFailTest(
							() => new List<int> { 1, 2 }.Should().SatisfyRespectively(value => value.Should().Be(1)),
	...
	{
		RunCompareFailTest(
							() => new List<int> { 1, 2 }.Should().SatisfyRespectively(value => value.Should().Be(1), value => value.Should().Be(5))
						);
	}
