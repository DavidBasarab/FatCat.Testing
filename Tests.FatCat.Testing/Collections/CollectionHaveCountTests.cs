		...
		RunCompareFailTest(
			() =>
				new List<int> { 1, 2, 3 }
					.Should()
					.HaveCount(5),
			"[1, 2, 3] should have count 5 but has 3"
		);
	}
	...
	public void BadHaveCountWithBecause()
	{
		RunCompareFailTest(
			() =>
				new List<int> { 1, 2, 3 }
					.Should()
					.HaveCount(5, "custom because"),
			"custom because"
		);
	}

	[Fact]
	public void GoodNotHaveCount()
	{
		new List<int> { 1, 2, 3 }
			.Should()
			.Not.HaveCount(5);
	...
	public void BadNotHaveCountWithBecause()
	{
		RunCompareFailTest(
			() =>
				new List<int> { 1, 2, 3 }
					.Should()
					.Not.HaveCount(3, "custom because"),
			"custom because"
		);
	}
