...
{
	[Fact]
	public void GoodBeSubsetOf()
	{
		new List<int> { 1, 2 }
			.Should()
			.BeSubsetOf(new List<int> { 1, 2, 3 });
	}

	[Fact]
	public void BadBeSubsetOf()
	{
		RunCompareFailTest(() =>
			new List<int> { 1, 5 }
				.Should()
	...
	public void BadBeSubsetOfWithBecause()
	{
		RunCompareFailTest(
			() =>
				new List<int> { 1, 5 }
					.Should()
					.BeSubsetOf(new List<int> { 1, 2, 3 }, "custom because"),
			"custom because"
		);
	}
	...
	public void BadBeSubsetOfOnNull()
	{
		...
		RunCompareFailTest(
			() => subject.Should().BeSubsetOf(new List<int> { 1, 2, 3 }),
			"null should be a subset of [1, 2, 3]"
		);
	}
}
