...
{
	[Fact]
	public void GoodContainSingle() { new List<int> { 1, 2, 3 }.Should().ContainSingle(value => value == 2); }

	[Fact]
	public void BadContainSingle() { RunCompareFailTest(() => new List<int> { 1, 2, 3 }.Should().ContainSingle(value => value > 1)); }

	[Fact]
	public void BadContainSingleShowsCorrectMessage()
	{
		RunCompareFailTest(
							() => new List<int> { 1, 2, 3 }.Should().ContainSingle(value => value > 1),
	...
	{
		RunCompareFailTest(() => new List<int> { 1, 2, 3 }.Should().ContainSingle(value => value > 1, "custom because"), "custom because");
	}
