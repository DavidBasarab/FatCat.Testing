...
{
	[Fact]
	public void GoodContainInOrder()
	{
		new List<int> { 1, 2, 3 }
			.Should()
	...
	[Fact]
	public void GoodContainInOrderNonContiguous()
	{
		new List<int> { 1, 2, 3, 4, 5 }
			.Should()
			.ContainInOrder(1, 3, 5);
	}

	[Fact]
	public void BadContainInOrderWhenOutOfOrder()
	{
		RunCompareFailTest(() =>
			new List<int> { 1, 2, 3 }
				.Should()
	...
	public void BadContainInOrderWithBecause()
	{
		RunCompareFailTest(
			() =>
				new List<int> { 1, 2, 3 }
					.Should()
					.ContainInOrder(new List<int> { 3, 1 }, "custom because"),
			"custom because"
		);
	}
