	...
	public void BadContainEquivalentOfWithBecause()
	{
		RunCompareFailTest(
			() =>
				new List<int> { 1, 2, 3 }
					.Should()
					.ContainEquivalentOf(5, "custom because"),
			"custom because"
		);
	}
	...
	public void BadNotContainEquivalentOfWithBecause()
	{
		RunCompareFailTest(
			() =>
				new List<int> { 1, 2, 3 }
					.Should()
					.Not.ContainEquivalentOf(2, "custom because"),
			"custom because"
		);
	}
