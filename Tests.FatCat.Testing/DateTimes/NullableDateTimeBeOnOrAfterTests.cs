...
public class NullableDateTimeBeOnOrAfterTests : BaseTest
{
	...
	public void BadBeOnOrAfterWithBecause()
	{
		...
		RunCompareFailTest(() => ((DateTime?)date).Should().BeOnOrAfter(laterDate, "custom because"), "custom because");
	}
	...
	public void BadNotBeOnOrAfterWithBecause()
	{
		...
		RunCompareFailTest(() => ((DateTime?)date).Should().Not.BeOnOrAfter(earlierDate, "custom because"), "custom because");
	}
