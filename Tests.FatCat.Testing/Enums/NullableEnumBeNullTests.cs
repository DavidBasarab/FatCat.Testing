...
public class NullableEnumBeNullTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((TestStatus?)TestStatus.Active).Should().BeNull("custom because"), "custom because");
	}
	...
	{
		RunCompareFailTest(() => ((TestStatus?)TestStatus.Active).Should().Not.HaveValue(), "Active should not have a value");
	}

	[Fact]
	public void BadNotHaveValueWithBecause()
	{
		RunCompareFailTest(() => ((TestStatus?)TestStatus.Active).Should().Not.HaveValue("custom because"), "custom because");
	}
