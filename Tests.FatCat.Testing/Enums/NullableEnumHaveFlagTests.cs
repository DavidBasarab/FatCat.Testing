...
public class NullableEnumHaveFlagTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => ((TestStatus?)null).Should().HaveFlag(TestStatus.Active), "null should have flag Active");
	}
	...
	{
		((TestStatus?)(TestStatus.Active | TestStatus.Pending)).Should().HaveFlag(TestStatus.Active);
	}

	[Fact]
	public void GoodNotHaveFlag()
	{
		((TestStatus?)TestStatus.Active).Should().Not.HaveFlag(TestStatus.Pending);
	}

	[Fact]
	public void GoodNotHaveFlagWhenNull()
	{
		((TestStatus?)null).Should().Not.HaveFlag(TestStatus.Active);
	}
}
