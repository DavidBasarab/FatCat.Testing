...
public class EnumHaveFlagTests : BaseTest
{
	...
	{
		RunCompareFailTest(() => TestStatus.Active.Should().HaveFlag(TestStatus.Pending), "Active should have flag Pending");
	}

	[Fact]
	public void BadHaveFlagWithBecause()
	{
		RunCompareFailTest(() => TestStatus.Active.Should().HaveFlag(TestStatus.Pending, "custom because"), "custom because");
	}

	[Fact]
	public void BadNotHaveFlag()
	{
		RunCompareFailTest(() => (TestStatus.Active | TestStatus.Pending).Should().Not.HaveFlag(TestStatus.Active));
	}
	...
	{
		RunCompareFailTest(
			() => (TestStatus.Active | TestStatus.Pending).Should().Not.HaveFlag(TestStatus.Active, "custom because"),
			"custom because"
		);
	}
