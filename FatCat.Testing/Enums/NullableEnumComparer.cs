	...
	where T : struct, Enum
{
	...
	{
		if (!Subject.HasValue || !Subject.Value.Equals(expected)) { CompareException.New(because ?? $"{SubjectDisplay} should be {expected}"); }
	...
	{
		if (!Subject.HasValue || !Enum.IsDefined(typeof(T), Subject.Value)) { CompareException.New(because ?? $"{SubjectDisplay} should be defined"); }
	...
	{
		if (!Subject.HasValue || !Subject.Value.HasFlag(expected)) { CompareException.New(because ?? $"{SubjectDisplay} should have flag {expected}"); }
