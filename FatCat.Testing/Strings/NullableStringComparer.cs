...
public class NullableStringComparer(string? subject) : ComparerBase<string?, NullableStringComparer>(subject)
{
	...
	public NullableStringComparer Be(string expected, Options options = Options.CaseSensitive, string? because = null)
	{
	...
	)
	{
		if (!StringEqualityHelper.AreEqual(Subject, expected, options))
		{
			CompareException.New(because ?? $"{SubjectDisplay} should be equivalent to {expected}");
		}
	...
	public NullableStringComparer Contain(string expected, Options options = Options.CaseSensitive, string? because = null)
	{
		if (StringEqualityHelper.CountOccurrences(Subject, expected, options) == 0)
		{
			CompareException.New(because ?? $"{SubjectDisplay} should contain {expected}");
		}
	...
	)
	{
		...
		if (!occurrence.IsSatisfiedBy(count))
		{
			CompareException.New(
				because ?? $"{SubjectDisplay} should contain {expected} {occurrence.Description()} but found {count}"
			);
		}
	...
	public NullableStringComparer ContainAll(string[] expected, Options options = Options.CaseSensitive, string? because = null)
	{
		var missing = expected.Where(e => StringEqualityHelper.CountOccurrences(Subject, e, options) == 0).ToList();

		if (missing.Count > 0)
		{
			CompareException.New(because ?? $"{SubjectDisplay} should contain all of [{string.Join(", ", missing)}]");
		}

		return this;
	}

	public NullableStringComparer ContainAny(string[] expected, Options options = Options.CaseSensitive, string? because = null)
	{
		var hasAny = expected.Any(e => StringEqualityHelper.CountOccurrences(Subject, e, options) > 0);

		if (!hasAny)
		{
			CompareException.New(because ?? $"{SubjectDisplay} should contain at least one of [{string.Join(", ", expected)}]");
		}
	...
	{
		if (!StringEqualityHelper.ContainsEquivalent(Subject, expected))
		{
			CompareException.New(because ?? $"{SubjectDisplay} should contain equivalent of {expected}");
		}

		return this;
	}

	public NullableStringComparer EndWith(string expected, Options options = Options.CaseSensitive, string? because = null)
	{
		if (Subject == null || !Subject.EndsWith(expected, StringEqualityHelper.ToComparison(options)))
		{
			CompareException.New(because ?? $"{SubjectDisplay} should end with {expected}");
		}
	...
	)
	{
		if (Subject == null || !Subject.EndsWith(expected, StringEqualityHelper.ToComparison(options)))
		{
			CompareException.New(because ?? $"{SubjectDisplay} should end with equivalent of {expected}");
		}
	...
	public NullableStringComparer HaveLength(int expected, string? because = null)
	{
		...
		{
			CompareException.New(because ?? $"{SubjectDisplay} should have length {expected}");
		}
	...
	public NullableStringComparer Match(string pattern, Options options = Options.CaseSensitive, string? because = null)
	{
		if (Subject == null || !StringEqualityHelper.MatchesWildcard(Subject, pattern, options))
		{
			CompareException.New(because ?? $"{SubjectDisplay} should match {pattern}");
		}
	...
	public NullableStringComparer MatchRegex(string pattern, string? because = null)
	{
		...
		{
			CompareException.New(because ?? $"{SubjectDisplay} should match regex {pattern}");
		}
	...
	public NullableStringComparer MatchRegex(Regex regex, string? because = null)
	{
		...
		{
			CompareException.New(because ?? $"{SubjectDisplay} should match regex {regex}");
		}

		return this;
	}

	public NullableStringComparer StartWith(string expected, Options options = Options.CaseSensitive, string? because = null)
	{
		if (Subject == null || !Subject.StartsWith(expected, StringEqualityHelper.ToComparison(options)))
		{
			CompareException.New(because ?? $"{SubjectDisplay} should start with {expected}");
		}
	...
	)
	{
		if (Subject == null || !Subject.StartsWith(expected, StringEqualityHelper.ToComparison(options)))
		{
			CompareException.New(because ?? $"{SubjectDisplay} should start with equivalent of {expected}");
		}
