	...
	where T : INumber<T>
{
	...
	{
		if (T.Abs(Subject - center) > tolerance) { CompareException.New(because ?? $"{Subject} should be around {center} within {tolerance}"); }
	...
	{
		if (Subject >= T.Zero) { CompareException.New(because ?? $"{Subject} should be negative"); }
	...
	{
		if (Subject <= T.Zero) { CompareException.New(because ?? $"{Subject} should be positive"); }
	...
	{
		if (Subject != T.Zero) { CompareException.New(because ?? $"{Subject} should be zero"); }
