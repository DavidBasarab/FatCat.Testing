...
public class CollectionComparer<T>(IEnumerable<T> subject) : ComparerBase<IEnumerable<T>, CollectionComparer<T>>(subject)
{
	...
	public CollectionComparer<T> Contain(T expected, string because = null)
	{
		...
		{
			CompareException.New(because ?? $"{FormatItems()} should contain {ValueFormatter.Format(expected)}");
		}
	...
	public CollectionComparer<T> BeEmpty(string because = null)
	{
		...
		{
			CompareException.New(because ?? $"{FormatItems()} should be empty");
		}
	...
	public CollectionComparer<T> HaveCount(int expected, string because = null)
	{
		...
		{
			CompareException.New(because ?? $"{FormatItems()} should have count {expected} but has {items?.Count ?? 0}");
		}
	...
	public CollectionComparer<T> ContainSingle(string because = null)
	{
		...
		{
			CompareException.New(because ?? $"{FormatItems()} should contain a single element but has {items?.Count ?? 0}");
		}
	...
	public CollectionComparer<T> ContainSingle(Func<T, bool> predicate, string because = null)
	{
		...
		if (items is null || matchedCount != 1)
		{
			CompareException.New(
				because ?? $"{FormatItems()} should contain a single element matching the predicate but {matchedCount} matched"
			);
		}
	...
	public CollectionComparer<T> Equal(IEnumerable<T> expected, string because = null)
	{
		...
		if (items is null || expectedItems is null || !items.SequenceEqual(expectedItems))
		{
			CompareException.New(because ?? $"{FormatItems()} should equal {ValueFormatter.Format(expectedItems)}");
		}
	...
	public CollectionComparer<T> ContainEquivalentOf(T expected, string because = null)
	{
		...
		{
			CompareException.New(
				because ?? $"{FormatItems()} should contain an element equivalent to {ValueFormatter.Format(expected)}"
			);
		}
	...
	{
		if (items is null || !items.All(predicate))
		{
			CompareException.New(because ?? $"{FormatItems()} should only contain elements matching the predicate");
		}
	...
	{
		if (items is null || items.Distinct().Count() != items.Count)
		{
			CompareException.New(because ?? $"{FormatItems()} should only have unique items");
		}
	...
	public CollectionComparer<T> ContainInOrder(IEnumerable<T> expected, string because = null)
	{
		...
		if (items is null || expectedItems is null || !ContainsInOrder(expectedItems))
		{
			CompareException.New(because ?? $"{FormatItems()} should contain {ValueFormatter.Format(expectedItems)} in order");
		}

		return this;
	}

	public CollectionComparer<T> ContainInOrder(params T[] expected)
	{
		return ContainInOrder((IEnumerable<T>)expected);
	}

	public CollectionComparer<T> BeInDescendingOrder(string because = null)
	{
		if (items is null || !IsInDescendingOrder())
		{
			CompareException.New(because ?? $"{FormatItems()} should be in descending order");
		}
	...
	{
		if (items is null || !IsInAscendingOrder())
		{
			CompareException.New(because ?? $"{FormatItems()} should be in ascending order");
		}
	...
	public CollectionComparer<T> BeSubsetOf(IEnumerable<T> superset, string because = null)
	{
		...
		if (items is null || supersetItems is null || items.Any(element => !supersetItems.Contains(element)))
		{
			CompareException.New(because ?? $"{FormatItems()} should be a subset of {ValueFormatter.Format(supersetItems)}");
		}
	...
	public CollectionComparer<T> IntersectWith(IEnumerable<T> other, string because = null)
	{
		...
		{
			CompareException.New(because ?? $"{FormatItems()} should intersect with {ValueFormatter.Format(otherItems)}");
		}
	...
	public CollectionComparer<T> AllSatisfy(Action<T> inspector, string because = null)
	{
		if (items is null)
		{
			CompareException.New(because ?? $"{FormatItems()} should have every element satisfy the inspector");
		}
		...
		if (failingIndex >= 0)
		{
			CompareException.New(
				because
					?? $"{FormatItems()} should have every element satisfy the inspector but element at index {failingIndex} did not"
			);
		}
	...
	{
		if (items is null || items.Count != inspectors.Length)
		{
			CompareException.New(
				$"{FormatItems()} should satisfy {inspectors.Length} inspectors respectively but has {items?.Count ?? 0} elements"
			);
		}
		...
		{
			CompareException.New(
				$"{FormatItems()} should satisfy {inspectors.Length} inspectors respectively but element at index {failingIndex} did not"
			);
		}
	...
	public CollectionComparer<T> AllBeOfType<TExpected>(string because = null)
	{
		return AllBeOfType(typeof(TExpected), because);
	}

	public CollectionComparer<T> AllBeOfType(Type expectedType, string because = null)
	{
		if (items is null || items.Any(element => element is null || element.GetType() != expectedType))
		{
			CompareException.New(because ?? $"{FormatItems()} should have all elements of type {expectedType}");
		}
	...
	{
		if (items is null || !items.All(element => EquivalencyComparer.Compare(element, expected).AreEquivalent))
		{
			CompareException.New(
				because ?? $"{FormatItems()} should have all elements equivalent to {ValueFormatter.Format(expected)}"
			);
		}
	...
	public CollectionComparer<T> HaveCountGreaterThan(int expected, string because = null)
	{
		...
		{
			CompareException.New(
				because ?? $"{FormatItems()} should have count greater than {expected} but has {items?.Count ?? 0}"
			);
		}
	...
	public CollectionComparer<T> HaveCountLessThan(int expected, string because = null)
	{
		...
		{
			CompareException.New(
				because ?? $"{FormatItems()} should have count less than {expected} but has {items?.Count ?? 0}"
			);
		}
	...
	public CollectionComparer<T> HaveSameCount(IEnumerable<T> other, string because = null)
	{
		...
		if (items is null || otherItems is null || items.Count != otherItems.Count)
		{
			CompareException.New(
				because
					?? $"{FormatItems()} should have the same count as {ValueFormatter.Format(otherItems)} ({otherItems?.Count ?? 0}) but has {items?.Count ?? 0}"
			);
		}
	...
	{
		if (items is null || index < 0 || index >= items.Count || !Equals(items[index], expected))
		{
			CompareException.New(because ?? $"{FormatItems()} should have {ValueFormatter.Format(expected)} at index {index}");
		}
	...
	public CollectionComparer<T> ContainNulls(string because = null)
	{
		...
		{
			CompareException.New(because ?? $"{FormatItems()} should contain nulls");
		}
	...
	public CollectionComparer<T> ContainMatch(string wildcard, string because = null)
	{
		if (
			items is null
			|| !items.Any(element =>
				element is string text && StringEqualityHelper.MatchesWildcard(text, wildcard, Options.CaseSensitive)
			)
		)
		{
			CompareException.New(because ?? $"{FormatItems()} should contain a match for {wildcard}");
		}
	...
	private bool IsInAscendingOrder()
	{
		...
		{
			if (comparer.Compare(items[index], items[index + 1]) > 0)
			{
				return false;
			}
		}
	...
	private string FindEquivalencyFailure(List<T> expectedItems)
	{
		...
		if (items is null || expectedItems is null)
		{
			return summary;
		}
		...
		{
			return $"{summary} but they have different counts ({items.Count} and {expectedItems.Count})";
		}
		...
		foreach (var actual in items)
		{
			...
			if (matchIndex < 0)
			{
				return $"{summary} but could not find a match for {ValueFormatter.Format(actual)}";
			}
	...
	private bool ContainsInOrder(IReadOnlyList<T> expected)
	{
		...
		{
			if (matchedCount < expected.Count && Equals(item, expected[matchedCount]))
			{
				matchedCount++;
			}
		}
