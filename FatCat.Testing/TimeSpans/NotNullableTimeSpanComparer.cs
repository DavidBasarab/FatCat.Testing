	...
	: NotComparerBase<TimeSpan?, NotNullableTimeSpanComparer>(subject)
{
	...
	{
		if (Subject.HasValue && Subject.Value == expected) { CompareException.New(because ?? $"{Subject.Value} should not be {expected}"); }
	...
	{
		if (Subject.HasValue && Math.Abs((Subject.Value - expected).Ticks) <= precision.Ticks) { CompareException.New(because ?? $"{Subject.Value} should not be within {precision} of {expected}"); }
	...
	{
		if (Subject.HasValue && Subject.Value >= expected) { CompareException.New(because ?? $"{Subject.Value} should not be greater than or equal to {expected}"); }
	...
	{
		if (Subject.HasValue && Subject.Value <= expected) { CompareException.New(because ?? $"{Subject.Value} should not be less than or equal to {expected}"); }
	...
	{
		if (Subject.HasValue && Subject.Value < TimeSpan.Zero) { CompareException.New(because ?? $"{Subject.Value} should not be negative"); }
	...
	{
		if (Subject.HasValue && Subject.Value > TimeSpan.Zero) { CompareException.New(because ?? $"{Subject.Value} should not be positive"); }
	...
	{
		if (Subject.HasValue && Subject.Value.Days == expected) { CompareException.New(because ?? $"{Subject.Value} should not have days {expected}"); }
	...
	{
		if (Subject.HasValue && Subject.Value.Hours == expected) { CompareException.New(because ?? $"{Subject.Value} should not have hours {expected}"); }
	...
	{
		if (Subject.HasValue && Subject.Value.Milliseconds == expected) { CompareException.New(because ?? $"{Subject.Value} should not have milliseconds {expected}"); }
	...
	{
		if (Subject.HasValue && Subject.Value.Minutes == expected) { CompareException.New(because ?? $"{Subject.Value} should not have minutes {expected}"); }
	...
	{
		if (Subject.HasValue && Subject.Value.Seconds == expected) { CompareException.New(because ?? $"{Subject.Value} should not have seconds {expected}"); }
